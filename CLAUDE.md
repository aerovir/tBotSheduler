# tBotSheduler — Telegram Bot для бронирования расписания

## Описание
Telegram-бот для канала, позволяющий админу создавать часовые слоты, а пользователям — бронировать их через Telegram Web App. При наступлении времени бот уведомляет пользователя.

## Стек технологий

| Компонент | Технология |
|-----------|-----------|
| Бот | `python-telegram-bot` (v20+, async) |
| Backend | FastAPI |
| База данных | SQLite + SQLAlchemy (async) + aiosqlite |
| Web App | Vanilla HTML/CSS/JS (Telegram Web App) |
| Планировщик | JobQueue (встроенный в python-telegram-bot) + Heartbeat на апдейтах |
| Деплой | Timeweb VPS (189 ₽/мес) — 1 vCPU, 1 GB RAM, 10 GB NVMe |
| Тестирование | pytest + pytest-asyncio + httpx + aioresponses |
| Webhook-доставка | `httpx.AsyncClient` (пул 10 соединений) |
| iCal | `icalendar` (pure Python) — экспорт/импорт .ics |
| Шифрование credentials | `cryptography` (Fernet) |
| Внешнее API | REST `/api/v1/*` — API Key аутентификация, in-memory rate limiter |

## Архитектура

```
┌─────────────────┐     ┌────────────────────────────────────────────┐     ┌─────────┐
│  Telegram Bot   │◄───►│              Backend (FastAPI)              │◄───►│ SQLite  │
│  (python-telegram-bot)│  ├─ Web App Routes (/api/*, initData auth) │     │ bot.db  │
└─────────────────┘     │  ├─ External API (/api/v1/*, API Key auth) │     └─────────┘
                         │  ├─ Webhook Dispatcher (event → POST)      │
                         │  └─ iCal Engine (import/export .ics)       │
                         └────────────────────────────────────────────┘
                                │                    │
                         ┌──────┴──────┐     ┌──────┴──────────┐
                         │  Telegram   │     │ Third-Party     │
                         │  Web App    │     │ API Clients     │
                         │ (HTML/JS)   │     │ + Calendars     │
                         └─────────────┘     └─────────────────┘
```

## Модели данных (SQLAlchemy)

- **Admin** — id, user_id (telegram), username, role (owner | moderator), added_by (admin_id), created_at
- **Channel** — id, chat_id, title, owner_id (FK→Admin), booking_horizon_days, default_notify_minutes
- **Slot** — id, channel_id (FK), date, start_time, end_time, is_active, created_by (admin_id)
- **Booking** — id, slot_id (FK), user_id (telegram), user_name, comment, created_at, notify_minutes
- **Notification** — id, booking_id (FK), user_id, notify_at (datetime), sent (bool), job_id (str)
- **AuditLog** — id, action (str), user_id, slot_id, booking_id, details (JSON), created_at
- **ApiClient** — id, admin_id (FK→Admin), name, key_hash (SHA-256), key_prefix, auth_method, permissions (JSON), rate_limit, is_active, expires_at (*)
- **Webhook** — id, admin_id (FK→Admin), channel_id (FK→Channel), url, events (JSON), secret, is_active, retry_max_attempts, consecutive_failures, is_degraded (*)
- **WebhookDeliveryLog** — id, webhook_id (FK→Webhook), event, delivery_id (UUID), payload (JSON), status, http_status, attempts, next_retry_at (*)
- **Integration** — id, admin_id, channel_id, integration_type, credentials_encrypted, sync_direction, config (JSON), last_sync_at (*)

> (*) Новые модели для внешнего API. Реализуются после Milestone 5.

### Роли администраторов

| Роль | Права | Может делегировать? |
|------|-------|:-------------------:|
| `owner` | Полный доступ: создание/удаление слотов, broadcast, /add_moderator, /remove_moderator, настройки канала, управление админами | ✅ |
| `moderator` | Ограниченный: создание/удаление слотов, broadcast, /free_slot, просмотр броней | ❌ |
| `developer` | Технический доступ: `/health` (healthcheck), `/logs` (последние N строк лога), `/version` (версия + uptime). Нет доступа к слотам/броням | ❌ |

**Кто становится owner:** пользователь, который выполнил `/setup` (привязал бота к каналу).
**Кто может стать moderator:** добавлен owner'ом через команду `/add_moderator`.
**Кто может стать developer:** добавлен owner'ом через команду `/add_developer`.

### Команды делегирования
- `/add_moderator <user_id>` — owner добавляет модератора
- `/remove_moderator <user_id>` — owner удаляет модератора
- `/moderators` — список всех модераторов канала
- `/add_developer <user_id>` — owner добавляет разработчика
- `/remove_developer <user_id>` — owner удаляет разработчика
- `/developers` — список разработчиков

### Команды разработчика
- `/health` — бот присылает текущий healthcheck в личное сообщение (форматированный текст)
- `/logs <lines>` — последние N строк лога (по умолчанию 20, макс 100)
- `/version` — версия бота, uptime, количество обработанных команд

### Индексы и ограничения
- `Booking`: уникальный индекс `(user_id, slot_id)` — защита от двойного нажатия на кнопку
- `Slot`: уникальный индекс `(channel_id, date, start_time)` — один слот не создать дважды
- `Notification`: индекс по `(notify_at, sent)` — быстрый поиск просроченных уведомлений

## Безопасность

### Критические меры (встраиваются с Milestone 1)

| Угроза | Мера | Где реализуется |
|--------|------|----------------|
| Доступ к VPS | SSH только по ключу, fail2ban, пользователь `tbot` вместо root | Деплой |
| Утечка токена | `.env` в `.gitignore`, права 600 на файл | Milestone 1 |
| Подделка hash в Web App | Валидация initData через HMAC-SHA256 с токеном бота | Milestone 2-3 |
| Race condition при брони | `select_for_update()` внутри транзакции | Milestone 3 |
| Повреждение SQLite | `PRAGMA journal_mode=WAL`, `busy_timeout=5000` | Milestone 1 |
| SQL-инъекции | Только параметризованные запросы через SQLAlchemy | Milestone 1 |
| Неавторизованный админ | Проверка роли (owner/moderator/developer) через таблицу Admin + кеш 5 мин | Milestone 2 |
| Модератор превышает права | Проверка `@check_role('owner')` для sensitive-команд (делегирование, настройки) | Milestone 2 |
| Developer превышает права | Проверка `@check_role('owner', 'moderator')` для команд управления слотами | Milestone 2 |
| Флуд бронированиями | Anti-flood: не чаще 1 действия / 5 сек на пользователя | Milestone 3 |
| Дублирование уведомлений | Флаг `sent = False` в Notification + идемпотентность | Milestone 3 |
| Rate limit Telegram | `DefaultRateLimiter` из python-telegram-bot | Milestone 1 |
| Утечка API-ключа | SHA-256 hash в БД, ключ показывается 1 раз при создании | External API |
| Утечка webhook secret | Автогенерация `secrets.token_hex(32)`, показывается 1 раз | External API |
| SSRF вебхуков | Только HTTPS, проверка internal IP (10.x, 172.16-31.x, 192.168.x, 127.x.x.x) | External API |
| Replay-атаки на API | Nonce + timestamp (HMAC-режим) / delivery UUID в вебхуках | External API |
| Brute-force API | Per-key rate limit (60 req/min) + IP rate limit (1000 req/min) | External API |
| Credentials календарей | Fernet-шифрование в БД (ключ из `INTEGRATION_ENCRYPTION_KEY` в .env) | External API |

### Обязательные паттерны в коде

```python
# 1. Все админ-команды — через декоратор @check_admin()
# 2. Все Web App запросы — validate_init_data() перед обработкой
# 3. Бронирование — внутри транзакции с select_for_update
# 4. SQLite — journal_mode=WAL, busy_timeout=5000 при подключении
# 5. .gitignore — .env, bot.db, __pycache__/
# 6. Rate limiter — включён DefaultRateLimiter при старте бота
# 7. Уведомления — идемпотентны (проверка sent = False)
# 8. AuditLog — логировать каждое изменение (создание слота, бронь, отмена)
# 9. Graceful shutdown — SIGTERM → остановить JobQueue → закрыть БД
# 10. Web App — проверять window.Telegram.WebApp.initDataUnsigned перед отправкой на бэкенд
```

## Логирование и обработка ошибок

### Уровни логирования
| Уровень | Куда пишем | Что попадает |
|---------|-----------|-------------|
| `ERROR` | stderr + файл `logs/error.log` | Ошибки БД, сбои JobQueue, исключения в хендлерах |
| `WARNING` | stderr + файл `logs/error.log` | Anti-flood сработал, невалидный initData, превышение лимитов |
| `INFO` | stdout | Бронирование, отмена, создание слота, отправка уведомления |
| `DEBUG` | stdout (только dev) | Все запросы к API, тело ответов, SQL-запросы |

### Стратегия при ошибках
1. **Ошибка БД** — пользователю: «Техническая ошибка, попробуйте позже». Админу в личку: «⚠️ Ошибка БД: ...»
2. **Ошибка JobQueue** — уведомление догонит Heartbeat при следующем апдейте. Логируем.
3. **Ошибка Telegram API** — повтор через 3 секунды (max 3 попытки). Если не удалось — логируем, пропускаем.
4. **Невалидный initData** — отклоняем запрос, логируем user_id и IP.

### Уведомление админа о сбоях
- Критические ошибки дублируются в личное сообщение админу
- Heartbeat при старте отчитывается: «Бот запущен, догнал X просроченных уведомлений»

## Graceful Shutdown

При получении `SIGTERM` / `SIGINT`:
1. Останавливаем принятие новых апдейтов (бот перестаёт отвечать)
2. JobQueue завершает текущие задачи (не запускает новые)
3. Закрываем все соединения с SQLite
4. Выходим с кодом 0

**Почему это важно:**
- JobQueue не теряет задачи, которые должны были сработать «вот-вот» — они попадут в БД со статусом `sent = False`
- SQLite не остаётся в «горячем» состоянии (WAL checkpoint)
- При перезапуске Heartbeat догонит всё, что не успело отправиться

```python
# Обработка сигналов (регистрируется при старте)
async def shutdown_handler(sig, frame):
    logger.info(f"Получен сигнал {sig}, начинаю graceful shutdown")
    await application.stop()   # остановить бота + JobQueue
    await application.shutdown()
    await engine.dispose()     # закрыть SQLAlchemy
    loop.stop()
```

## Ограничения SQLite и стратегия роста

### Когда SQLite работает хорошо
- До ~1000 активных пользователей в день
- До ~50 одновременных запросов на запись
- Объём данных до ~1 GB (миллионы броней)

### Когда нужен переход на PostgreSQL
- Одновременных бронирований > 20-30 в секунду (начинаются `database is locked`)
- Планируется шардирование или репликация
- Требуется более гибкая система прав доступа

### Стратегия миграции (если понадобится)
1. Все запросы через SQLAlchemy — ORM абстрагирует БД
2. Меняется только `DATABASE_URL` в `.env`:
   - `sqlite+aiosqlite:///bot.db` → `postgresql+asyncpg://user:pass@host/db`
3. WAL-специфичные прагмы убираются, PostgreSQL использует свои настройки
4. Рекомендуемый провайдер при миграции: **Timeweb PostgreSQL** (от 150 ₽/мес) или **Selectel Managed DB**

## Web App — защита API-эндпоинтов

### Проверка на клиенте (JS)
```javascript
// Проверяем, что Web App открыт внутри Telegram
if (!window.Telegram || !window.Telegram.WebApp) {
    document.body.innerHTML = 'Это приложение доступно только внутри Telegram';
    throw new Error('Not in Telegram');
}
const initData = window.Telegram.WebApp.initData;  // строка для отправки на бэкенд
```

### Проверка на сервере (Python)
```python
from hashlib import sha256
import hmac
import urllib.parse

def validate_init_data(init_data: str, bot_token: str) -> dict | None:
    """Проверяет подпись initData из Telegram Web App."""
    parsed = dict(urllib.parse.parse_qsl(init_data))
    hash_received = parsed.pop('hash', None)
    if not hash_received:
        return None
    
    secret_key = hmac.new(
        b'WebAppData', bot_token.encode(), sha256
    ).digest()
    
    data_check_string = '\n'.join(
        f'{k}={v}' for k, v in sorted(parsed.items())
    )
    expected_hash = hmac.new(
        secret_key, data_check_string.encode(), sha256
    ).hexdigest()
    
    if hmac.compare_digest(expected_hash, hash_received):
        return parsed  # валидно — возвращаем данные пользователя
    return None
```

### Защита от внешних запросов
- Все API-эндпоинты Web App проверяют initData
- Эндпоинты без initData (`/health`) не раскрывают пользовательских данных
- CORS настроен только на домен Telegram (или отключён для Web App)

## Healthcheck — мониторинг критических узлов

Единый эндпоинт `/health` возвращает статус всех компонентов системы. Используется для:
- Мониторинга в панели Timeweb (HTTP-алерты)
- Проверки после деплоя (CI/CD)
- Быстрой диагностики при сбоях

### Формат ответа

```json
{
  "status": "ok",
  "version": "1.0.0",
  "uptime_seconds": 3600,
  "checks": {
    "database": {
      "status": "ok",
      "detail": "wal_mode=ON, busy_timeout=5000, size_mb=2.3"
    },
    "bot": {
      "status": "ok",
      "detail": "running, job_queue_size=12"
    },
    "telegram_api": {
      "status": "ok",
      "detail": "latency_ms=210"
    },
    "disk": {
      "status": "ok",
      "detail": "free_mb=5120, total_mb=10240"
    },
    "memory": {
      "status": "ok",
      "detail": "rss_mb=145, available_mb=512"
    }
  }
}
```

### Критические узлы (check-функции)

| Узел | Что проверяет | Статусы |
|------|--------------|---------|
| `database` | Пинг SQLite, PRAGMA journal_mode, размер файла, busy_timeout | `ok` / `degraded` / `down` |
| `bot` | Application.runner работает, JobQueue размер, кол-во активных задач | `ok` / `degraded` (заタスク >1000) / `down` |
| `telegram_api` | GET `api.telegram.org/getMe`, замер latency | `ok` / `degraded` (latency >2s) / `down` |
| `disk` | Свободное место на диске, размер bot.db | `ok` / `degraded` (<1GB free) / `down` |
| `disk` | Свободное место на диске, размер bot.db | `ok` / `degraded` (<1GB free) / `down` |
| `memory` | RSS процесса, доступная RAM | `ok` / `degraded` (<200MB available) / `down` |
| `scheduler` | Heartbeat, кол-во pending Notification, последний чек | `ok` / `degraded` (нет чека >10мин) |
| `web_app` | Статика Web App доступна (проверка index.html) | `ok` / `down` |

### Реализация

```python
# app/core/health.py
from dataclasses import dataclass
from enum import Enum

class HealthStatus(str, Enum):
    OK = "ok"
    DEGRADED = "degraded"
    DOWN = "down"

@dataclass
class HealthCheck:
    name: str
    status: HealthStatus
    detail: str

async def check_database() -> HealthCheck:
    try:
        async with engine.connect() as conn:
            # Пинг
            await conn.execute(text("SELECT 1"))
            # PRAGMA journal_mode
            row = await conn.execute(text("PRAGMA journal_mode"))
            mode = row.scalar()
            
            # Размер файла
            size = os.path.getsize(DB_PATH)
            
            status = HealthStatus.OK
            if mode != "wal":
                detail = f"journal_mode={mode} (expected wal)"
                status = HealthStatus.DEGRADED
            else:
                detail = f"wal_mode=ON, size_mb={size / 1e6:.1f}"
                
    except Exception as e:
        return HealthCheck("database", HealthStatus.DOWN, str(e))
    
    return HealthCheck("database", status, detail)


async def check_bot(application) -> HealthCheck:
    if not application.running:
        return HealthCheck("bot", HealthStatus.DOWN, "bot is not running")
    
    jq = application.job_queue
    queue_size = len(jq.jobs()) if jq else 0
    
    status = HealthStatus.OK
    detail = f"running, job_queue_size={queue_size}"
    
    if queue_size > 1000:
        status = HealthStatus.DEGRADED
        detail += " (queue > 1000)"
    
    return HealthCheck("bot", status, detail)


async def check_telegram_api(bot) -> HealthCheck:
    import time
    start = time.monotonic()
    try:
        me = await bot.get_me()
        latency = (time.monotonic() - start) * 1000
        
        status = HealthStatus.OK
        detail = f"latency_ms={int(latency)}, bot_name={me.username}"
        
        if latency > 2000:
            status = HealthStatus.DEGRADED
            detail += " (high latency)"
    except Exception as e:
        return HealthCheck("telegram_api", HealthStatus.DOWN, str(e))
    
    return HealthCheck("telegram_api", status, detail)


async def check_disk() -> HealthCheck:
    st = shutil.disk_usage("/")
    free_mb = st.free / 1e6
    total_mb = st.total / 1e6
    
    status = HealthStatus.OK
    detail = f"free_mb={int(free_mb)}, total_mb={int(total_mb)}"
    
    if free_mb < 1000:
        status = HealthStatus.DEGRADED
        detail += " (disk space < 1GB)"
    
    return HealthCheck("disk", status, detail)


async def check_memory() -> HealthCheck:
    import psutil
    process = psutil.Process()
    rss_mb = process.memory_info().rss / 1e6
    available = psutil.virtual_memory().available / 1e6
    
    status = HealthStatus.OK
    detail = f"rss_mb={int(rss_mb)}, available_mb={int(available)}"
    
    if available < 200:
        status = HealthStatus.DEGRADED
        detail += " (low memory)"
    
    return HealthCheck("memory", status, detail)


async def check_scheduler(db_session) -> HealthCheck:
    """Проверка Heartbeat: нет ли зависших уведомлений."""
    try:
        pending = await db_session.execute(
            text("SELECT COUNT(*) FROM notification WHERE sent = 0 AND notify_at <= datetime('now')")
        )
        count = pending.scalar()
        
        status = HealthStatus.OK if count == 0 else HealthStatus.DEGRADED
        detail = f"pending_notifications={count}"
        
        return HealthCheck("scheduler", status, detail)
    except Exception as e:
        return HealthCheck("scheduler", HealthStatus.DOWN, str(e))


# FastAPI эндпоинт
@router.get("/health")
async def health(request: Request):
    start = time.monotonic()
    
    # Запускаем проверки параллельно
    checks = await asyncio.gather(
        check_database(),
        check_bot(request.app.state.bot_app),
        check_telegram_api(request.app.state.bot_app.bot),
        check_disk(),
        check_memory(),
        check_scheduler(request.app.state.db_session),
        return_exceptions=True
    )
    
    # Фильтруем ошибки
    results = []
    for c in checks:
        if isinstance(c, Exception):
            results.append(HealthCheck("unknown", HealthStatus.DOWN, str(c)))
        else:
            results.append(c)
    
    # Общий статус
    has_down = any(c.status == HealthStatus.DOWN for c in results)
    has_degraded = any(c.status == HealthStatus.DEGRADED for c in results)
    
    if has_down:
        overall = "down"
    elif has_degraded:
        overall = "degraded"
    else:
        overall = "ok"
    
    return {
        "status": overall,
        "version": get_version(),
        "uptime_seconds": int(time.monotonic() - request.app.state.started_at),
        "response_time_ms": int((time.monotonic() - start) * 1000),
        "checks": {c.name: {"status": c.status, "detail": c.detail} for c in results},
    }
```

### Где вызывается healthcheck

| Место | Частота | Формат |
|-------|---------|--------|
| HTTP `/health` | По запросу (мониторинг Timeweb) | JSON |
| При старте бота | 1 раз | Лог + уведомление админу |
| Heartbeat (каждый апдейт) | На каждый апдейт | Лог, только если статус не OK |
| Cron каждые 5 минут | Периодически | Лог (для собственных метрик) |

```python
# Вызов healthcheck при старте бота (lifespan startup)
async def on_startup():
    health_data = await run_healthcheck()
    if health_data["status"] != "ok":
        logger.warning(f"Healthcheck at startup: {health_data['status']}")
        for name, check in health_data["checks"].items():
            if check["status"] != "ok":
                logger.warning(f"  {name}: {check['status']} — {check['detail']}")
    else:
        logger.info("All health checks passed at startup")


### Отправка healthcheck в Telegram

Команда `/health` доступна ролям `developer` и `owner`. Бот форматирует результат в читаемое сообщение.

```python
# app/core/health.py (продолжение)

EMOJI_MAP = {
    HealthStatus.OK: "✅",
    HealthStatus.DEGRADED: "⚠️",
    HealthStatus.DOWN: "❌",
}

def format_health_message(health_data: dict) -> str:
    """Форматирует результат healthcheck для отправки в Telegram."""
    overall = health_data["status"]
    overall_emoji = {
        "ok": "✅", "degraded": "⚠️", "down": "❌"
    }.get(overall, "❓")
    
    lines = [
        f"<b>🏥 Healthcheck</b>\n",
        f"<b>Статус:</b> {overall_emoji} {overall.upper()}",
        f"<b>Версия:</b> {health_data['version']}",
        f"<b>Uptime:</b> {format_uptime(health_data['uptime_seconds'])}",
        f"<b>Время ответа:</b> {health_data['response_time_ms']} мс\n",
        "<b>Проверки:</b>"
    ]
    
    for name, check in health_data["checks"].items():
        emoji = EMOJI_MAP.get(check["status"], "❓")
        name_label = {
            "database": "🗄️ БД",
            "bot": "🤖 Бот",
            "telegram_api": "📡 Telegram API",
            "disk": "💾 Диск",
            "memory": "🧠 Память",
            "scheduler": "⏰ Планировщик",
        }.get(name, name)
        
        lines.append(f"{emoji} <b>{name_label}</b>: {check['detail']}")
    
    return "\n".join(lines)


def format_uptime(seconds: int) -> str:
    days = seconds // 86400
    hours = (seconds % 86400) // 3600
    minutes = (seconds % 3600) // 60
    secs = seconds % 60
    parts = []
    if days: parts.append(f"{days}д")
    if hours: parts.append(f"{hours}ч")
    if minutes: parts.append(f"{minutes}м")
    parts.append(f"{secs}с")
    return " ".join(parts)


# Хендлер команды /health
async def health_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Отправляет healthcheck в личное сообщение."""
    user_id = update.effective_user.id
    
    # Проверка прав: только developer или owner
    if not await user_has_role(user_id, ["developer", "owner"]):
        await update.message.reply_text("⛔ У вас нет прав для этой команды.")
        return
    
    # Запускаем проверки
    health_data = await run_healthcheck()
    
    message = format_health_message(health_data)
    
    # Отправляем в личку, если команда из группы/канала
    if update.effective_chat.type != "private":
        await update.message.reply_text(
            "📋 Результат отправлен в личные сообщения."
        )
        await context.bot.send_message(
            chat_id=user_id,
            text=message,
            parse_mode="HTML"
        )
    else:
        await update.message.reply_text(message, parse_mode="HTML")
```

### Пример ответа в Telegram

```
🏥 Healthcheck

Статус: ✅ OK
Версия: 1.0.0
Uptime: 3д 12ч 45м 10с
Время ответа: 45 мс

Проверки:
✅ 🗄️ БД: wal_mode=ON, size_mb=2.3
✅ 🤖 Бот: running, job_queue_size=5
✅ 📡 Telegram API: latency_ms=210, bot_name=@my_bot
✅ 💾 Диск: free_mb=5120, total_mb=10240
✅ 🧠 Память: rss_mb=145, available_mb=512
✅ ⏰ Планировщик: pending_notifications=0
```
```

Логируются все ключевые операции для возможности отката изменений и разбора инцидентов.

| Действие | audit_log.action | Детали |
|----------|-----------------|--------|
| Создание слота | `slot_created` | admin_id, date, start_time, end_time |
| Удаление слота | `slot_deleted` | admin_id, slot_id |
| Бронирование | `booking_created` | user_id, slot_id, notify_minutes |
| Отмена брони | `booking_cancelled` | user_id, slot_id, cause (user/admin) |
| Изменение брони | `booking_changed` | user_id, old_slot_id, new_slot_id |
| Уведомление отправлено | `notification_sent` | booking_id, notify_at |
| Попытка взлома | `security_alert` | user_id, reason (invalid_hash, flood, etc.) |

```sql
-- AuditLog не влияет на производительность — вставки идут асинхронно, без ожидания
INSERT INTO audit_log (action, user_id, slot_id, booking_id, details)
VALUES ('booking_created', 12345, 678, 910, '{"notify_minutes": 10}');
```

## Защита от «забытых» броней

Пользователь забронировал слот, но не пришёл — слот простаивает.

### Автоматические правила
- **N часов до начала:** если пользователь не взаимодействовал с ботом > 24ч → бот отправляет запрос на подтверждение. Если нет ответа за 1 час → бронь отменяется, слот свободен.
- **После начала слота:** если прошло 15 минут, а пользователь не отметился → админ получает уведомление, может форсированно освободить слот.

### Для админа
- Команда `/free_slot <id>` — принудительно освободить слот
- Команда `/bookings` — посмотреть все активные брони с таймером «сколько осталось до начала»

## External API — интеграция со сторонними сервисами

**Статус:** запроектировано, реализация после завершения Milestone 1–5.

### Компоненты

| Компонент | Неймспейс | Назначение |
|-----------|-----------|-----------|
| REST API | `/api/v1/*` | Полный CRUD для слотов, броней, каналов |
| Исходящие вебхуки | Event → POST | Уведомление внешних сервисов при изменениях |
| iCal экспорт | `GET /api/v1/schedule.ics` | Выгрузка расписания в календарь |
| iCal импорт | `POST /api/v1/schedule.ics` | Создание слотов из .ics |
| Календари (позже) | — | Google Calendar / Outlook bridge |

### Маршруты REST API

| Метод | Endpoint | Описание | Permission |
|-------|----------|----------|-----------|
| GET | `/api/v1/slots` | Список слотов (фильтр по дате) | `slots:r` |
| POST | `/api/v1/slots` | Создание одного или нескольких слотов | `slots:w` |
| GET | `/api/v1/slots/{id}` | Слот со статусом брони | `slots:r` |
| PUT | `/api/v1/slots/{id}` | Обновление слота | `slots:w` |
| DELETE | `/api/v1/slots/{id}` | Удаление слота (каскад на брони) | `slots:w` |
| GET | `/api/v1/bookings` | Список броней (фильтр по слоту/дате/пользователю) | `bookings:r` |
| GET | `/api/v1/bookings/{id}` | Детали брони | `bookings:r` |
| POST | `/api/v1/bookings` | Создание брони от имени пользователя | `bookings:w` |
| DELETE | `/api/v1/bookings/{id}` | Отмена брони | `bookings:w` |
| GET | `/api/v1/channels` | Список каналов админа | `channels:r` |
| PUT | `/api/v1/channels/{id}` | Обновление настроек канала | `channels:w` |

### Аутентификация

Обсуждается три варианта (финальное решение — при реализации):

| Вариант | Механика | Когда выбрать |
|---------|----------|---------------|
| **API Key** (рекомендуется) | `X-API-Key: sk_live_xxx`. SHA-256 в БД | M2M-взаимодействие, простота |
| **API Key + HMAC** | Ключ + HMAC-подпись тела запроса (X-API-Signature) | Высокие требования к безопасности, защита от replay |
| **JWT** | Токен с sub/scope/exp, stateless | Если появятся пользовательские логины |

Формат ключа: `sk_live_<32-байта-hex>`.  
Хранение: SHA-256(key) — plaintext нигде не хранится.  
Права доступа: JSON-поле `permissions` в модели ApiClient: `{"slots": "rw", "bookings": "r", "channels": "r"}`.  
Метод проверки: `api_client.has_permission(resource, access)`.

### Вебхуки — исходящие события

При изменениях в системе бот отправляет POST на зарегистрированные URL внешних сервисов.

**События:**
- `booking.created` — создана бронь
- `booking.cancelled` — бронь отменена
- `booking.changed` — бронь изменена (другой слот/время)
- `slot.created` — создан слот
- `slot.deleted` — слот удалён
- `slot.updated` — слот изменён

**Доставка:**
```
Событие (напр. booking.created)
    ↓
WebhookDispatcher находит активные вебхуки, подписанные на это событие
    ↓
Для каждого вебхука:
    ├─ Подпись тела: HMAC-SHA256(secret, body)
    ├─ POST с заголовками:
    │   ├─ X-Webhook-ID
    │   ├─ X-Webhook-Event: booking.created
    │   ├─ X-Webhook-Signature: sha256=<hex>
    │   ├─ X-Webhook-Delivery: <UUID>  (idempotency)
    │   └─ X-Webhook-Timestamp
    ├─ 2xx → success
    └─ non-2xx/timeout → retry
```

**Retry-стратегия:**

| Попытка | Задержка | Всего прошло |
|---------|----------|-------------|
| 1 | 0s | 0s |
| 2 | 10s | 10s |
| 3 | 1 min | 70s |
| 4 | 5 min | ~6 min |
| 5 | 15 min | ~21 min |

После 5 неудач — флаг `is_degraded = True`. Админ получает уведомление.  
Retry работает через background asyncio-task, опрашивающий `WebhookDeliveryLog`.

**Защита от SSRF:** URL вебхука проверяется при создании — только HTTPS, запрещены internal IP (10.x, 172.16-31.x, 192.168.x, 127.x.x.x).

### iCal — экспорт и импорт

**Экспорт** — `GET /api/v1/schedule.ics?channel_id=X&from=2026-01-01&to=2026-12-31`
- Генерирует валидный `.ics` (RFC 5545) через библиотеку `icalendar`.
- Каждый слот = VEVENT с DTSTART/DTEND, SUMMARY, DESCRIPTION (статус брони).
- Content-Type: `text/calendar`.

**Импорт** — `POST /api/v1/schedule.ics` с телом `.ics`
- Парсинг VEVENT, создание слотов в БД.
- Валидация: проверка дубликатов, пересечения времени, sanity дат.
- Ответ: `{"created": N, "skipped": M, "errors": [...]}`.

**Google Calendar / Outlook** — deferred. Базовый класс `CalendarIntegration` (ABC) подготовлен, конкретные реализации — по необходимости.

### Rate Limiting

- **Механизм:** in-memory sliding window (asyncio.Lock + deque).
- **Default:** 60 запросов/мин на ключ (настраивается в `ApiClient.rate_limit`).
- **IP limit:** 1000 запросов/мин/IP (поверх per-key).
- **Без Redis** — overhead ~150 KB для 100 ключей × 60 записей.
- **Cleanup:** каждые 5 мин удаляются пустые окна старше 1 часа.
- Middleware на все `/api/v1/*` запросы.

### Модели данных для внешнего API (SQLAlchemy)

**ApiClient:**
```python
class ApiClient(Base):
    __tablename__ = "api_client"
    id, admin_id (FK→Admin), name, key_hash (SHA-256, unique)
    key_prefix (первые 8 символов ключа для идентификации)
    auth_method ("api_key" | "hmac")
    permissions (JSON: {"slots": "rw", "bookings": "r", ...})
    rate_limit (int, default 60), is_active (bool), expires_at (DateTime, nullable)
    last_used_at, created_at, updated_at

    def has_permission(self, resource, access) -> bool
```

**Webhook:**
```python
class Webhook(Base):
    __tablename__ = "webhook"
    id, admin_id (FK→Admin), channel_id (FK→Channel, nullable)
    url, events (JSON: ["booking.created", ...])
    secret (auto-generated), is_active
    retry_max_attempts (default 5)
    last_delivery_at, last_delivery_status
    consecutive_failures, is_degraded
    created_at, updated_at
```

**WebhookDeliveryLog:**
```python
class WebhookDeliveryLog(Base):
    __tablename__ = "webhook_delivery_log"
    id, webhook_id (FK→Webhook), event, delivery_id (UUID, unique)
    payload (JSON), status (pending|delivered|failed)
    http_status, response_body (Text), attempts
    next_retry_at, created_at, delivered_at
```

**Integration:**
```python
class Integration(Base):
    __tablename__ = "integration"
    id, admin_id (FK→Admin), channel_id (FK→Channel, nullable)
    integration_type ("ical_export"|"ical_import"|"google_calendar"|"outlook")
    credentials_encrypted (Text, Fernet), sync_direction (import|export|bidirectional)
    config (JSON), is_active, last_sync_at, last_sync_status
```

### Команды управления

Доступны owner'у через Telegram после реализации:

| Команда | Описание |
|---------|----------|
| `/api_keys` | Список API-ключей |
| `/create_api_key <name>` | Создать новый ключ (показывается 1 раз) |
| `/revoke_api_key <id>` | Деактивировать ключ |
| `/webhooks` | Список вебхуков |
| `/add_webhook <url> <events>` | Зарегистрировать вебхук (events через запятую) |
| `/remove_webhook <id>` | Удалить вебхук |
| `/test_webhook <id>` | Отправить тестовое событие |
| `/integrations` | Список календарных интеграций |
| `/add_integration <type>` | Добавить интеграцию |

### Архитектура модулей

```
app/
├── api/
│   ├── external/
│   │   ├── __init__.py
│   │   ├── router.py        # Агрегация всех sub-router под /api/v1
│   │   ├── deps.py          # Dependency injection: get_api_client, verify_permission
│   │   ├── slots.py         # /api/v1/slots
│   │   ├── bookings.py      # /api/v1/bookings
│   │   ├── channels.py      # /api/v1/channels
│   │   └── health.py        # /api/v1/health (без аутентификации)
├── webhook/
│   ├── __init__.py
│   ├── dispatcher.py        # Async dispatch engine
│   ├── signing.py           # HMAC-SHA256 подпись + верификация
│   └── retry.py             # Retry queue + exponential backoff
├── integration/
│   ├── __init__.py
│   ├── ical.py              # Генерация + парсинг .ics
│   └── base.py              # ABC для Google/Outlook (deferred)
└── core/
    └── rate_limiter.py      # In-memory sliding window per API key
```

### Безопасность

| Угроза | Мера |
|--------|------|
| Утечка API-ключа | SHA-256 hash в БД, ключ показывается 1 раз. Key prefix (`sk_live_ab...`) — в списке |
| Replay-атаки | (Опционально) X-API-Timestamp + X-API-Nonce, окно 5 мин |
| Brute-force | Per-key rate limit (60/min) + IP rate limit (1000/min) |
| Просроченные ключи | `expires_at` → 403 |
| SSRF вебхуков | Только HTTPS, запрещены internal IP |
| Подделка payload вебхука | `X-Webhook-Signature: sha256=<HMAC-SHA256>` — потребитель верифицирует |
| Утечка webhook secret | Автогенерация, показывается 1 раз |
| Credentials календарей | Fernet-шифрование, ключ из `INTEGRATION_ENCRYPTION_KEY` в .env |
| Инъекции | Валидация всех входных данных через Pydantic |
| Аудит | Каждый API-запрос — в AuditLog (api_client_id, method, endpoint, IP, status) |

### Конфигурация (.env, добавления)

```
INTEGRATION_ENCRYPTION_KEY=ключ_fernet_для_шифрования_credentials
EXTERNAL_API_CORS_ORIGINS=*  # или список разрешённых origins
```

### Тестирование внешнего API

**Unit-тесты:**
- Создание моделей ApiClient, Webhook, WebhookDeliveryLog, Integration
- `has_permission()` — проверка разрешений
- Rate limiter: 60 запросов прошло, 61-й — блок, окно сдвигается
- Webhook signing: правильная подпись → verify, неправильная → reject
- SSRF-валидация: приватные IP блокируются

**Интеграционные:**
- CRUD API-ключей через Telegram команды
- External API auth: валидный ключ → 200, невалидный → 401, истёкший → 403
- Permission check: read-only ключ пробует POST → 403
- Rate limiter: превышение лимита → 429
- Webhook delivery на событие `booking.created` (mock aioresponses)
- Retry: 500 → retry → 200 → delivered
- Degradation: 5 неудач → is_degraded

**E2E:**
- Полный сценарий: API-ключ → создание слота → бронь → вебхук → отмена → вебхук
- iCal: экспорт → парсинг → валидация .ics → импорт → проверка слотов

## Планировщик уведомлений

### Механизм A — JobQueue (основной, точный)
Встроенный JobQueue python-telegram-bot. При бронировании регистрируется задача `run_once` на время `start_time - notify_minutes`.

### Механизм B — Heartbeat (страховочный)
При каждом апдейте от пользователя (команда, кнопка, Web App) и при старте бота — проверка просроченных непосланных уведомлений в БД.

```
Бронирование → notify_at = start_time - notify_minutes
    ↓
JobQueue.run_once(callback, when=notify_at) + запись в SQLite
    ↓
В notify_at → JobQueue шлёт уведомление → sent = True
    ↓
При перезагрузке → check_pending() при старте догоняет просрочку
    ↓
При любом апдейте → check_pending() как страховка
```

## Команды бота

### Админ
- `/setup` — привязать бота к каналу
- `/create_slots` — открыть Web App для создания слотов (даты + часы)
- `/slots` — просмотр/редактирование слотов
- `/settings` — настройки: горизонт бронирования, время уведомлений
- `/broadcast` — отправить сообщение с кнопкой "📅 Забронировать" в канал

### Пользователи
- Кнопка "📅 Забронировать" в канале → Web App
- Web App: календарь → дата → свободные часы → бронирование
- Мои брони: просмотр, отмена, изменение
- Таблица: кто занял какие слоты (дата, время, пользователь)

## UX Flow

```
Админ → /create_slots → Web App (выбор дат и часов)
    ↓
Бот публикует сообщение с кнопкой "📅 Забронировать"
    ↓
Пользователь → кнопка → Web App
    ↓
Web App: календарь → дата → свободные часы → бронь
    ↓
JobQueue регистрирует задачу через run_once
    ↓
Бот → подтверждение в личку
    ↓
В notify_at → JobQueue → уведомление в личку
    ↓
При любом апдейте → Heartbeat check_pending()
```

## Этапы реализации

### Этап 1 — Основа
- [ ] Инициализация проекта (FastAPI + бот)
- [ ] Модели SQLAlchemy + SQLite
- [ ] Базовый каркас: запуск бота и API
- [ ] Команда `/start`

### Этап 2 — Админ-панель
- [ ] Проверка прав админа
- [ ] Команды: `/setup`, `/create_slots`
- [ ] Web App для создания слотов (выбор даты, выбор часов)
- [ ] Сохранение слотов в БД
- [ ] `/broadcast` — публикация кнопки бронирования в канал

### Этап 3 — Бронирование
- [ ] Web App для пользователей: календарь, список свободных слотов
- [ ] Логика бронирования (запись в Booking + Notification)
- [ ] JobQueue: регистрация уведомления
- [ ] Heartbeat: check_pending() при старте и на апдейтах
- [ ] Подтверждение брони (личное сообщение)
- [ ] Таблица занятых слотов (кто, когда, какое время)

### Этап 4 — Отмена и изменение
- [ ] Отмена брони (удаление Notification + JobQueue.remove_job)
- [ ] Изменение слота (пересоздание уведомления)
- [ ] Оповещение админа об отмене (опционально)

### Этап 5 — Полировка и доп. фичи
- [ ] Временные зоны пользователей
- [ ] Очередь ожидания на занятые слоты
- [ ] Экспорт расписания
- [ ] Dockerfile + docker-compose (опционально)

### Этап 6 — External API (после этапов 1–5)
- [ ] Модели ApiClient, Webhook, WebhookDeliveryLog, Integration
- [ ] In-memory rate limiter (sliding window)
- [ ] Telegram команды: `/api_keys`, `/create_api_key`, `/revoke_api_key`
- [ ] Telegram команды: `/webhooks`, `/add_webhook`, `/remove_webhook`, `/test_webhook`
- [ ] REST API scaffold: `/api/v1/*` роутер, auth dependency, channels
- [ ] REST API: slots endpoints (CRUD)
- [ ] REST API: bookings endpoints (CRUD)
- [ ] Webhook dispatcher + signing + retry
- [ ] iCal экспорт + импорт
- [ ] Integration commands + календарный bridge
- [ ] Аудит API-запросов (AuditLog)

## Конфигурация (.env)

```
BOT_TOKEN=токен_бота
CHANNEL_USERNAME=@название_канала
WEB_APP_URL=https://домен/webapp
DATABASE_URL=sqlite+aiosqlite:///bot.db

# External API (опционально)
INTEGRATION_ENCRYPTION_KEY=ключ_fernet_для_шифрования_credentials
EXTERNAL_API_CORS_ORIGINS=*
```

## Запуск (dev)

```bash
pip install -r requirements.txt
python -m app.main
```

## Деплой на Timeweb (189 ₽/мес)

### Тариф
**Timeweb VPS «Start»** — 189 ₽/мес (акция, далее ~350 ₽):
- 1 vCPU, 1 GB RAM, 10 GB NVMe
- 100 Mbps, безлимитный трафик
- Дата-центр: Москва или Санкт-Петербург
- Оплата: MIR, СБП, USDT (TRC-20)

### Подготовка VPS
```bash
# Подключение
ssh root@<ip-сервера>

# Обновление системы
apt update && apt upgrade -y

# Установка Python и зависимостей
apt install -y python3 python3-venv python3-pip git
```

### Развёртывание
```bash
# Клонирование репозитория
git clone <url-репозитория> /opt/tBotSheduler
cd /opt/tBotSheduler

# Виртуальное окружение
python3 -m venv venv
source venv/bin/activate
pip install -r requirements.txt

# Создание .env
cp .env.example .env
nano .env  # BOT_TOKEN, CHANNEL_USERNAME, WEB_APP_URL
```

### systemd сервис
```ini
# /etc/systemd/system/tbotsheduler.service
[Unit]
Description=tBotSheduler
After=network.target

[Service]
Type=simple
User=root
WorkingDirectory=/opt/tBotSheduler
ExecStart=/opt/tBotSheduler/venv/bin/python -m app.main
Restart=always
RestartSec=10
Environment="PATH=/opt/tBotSheduler/venv/bin"
EnvironmentFile=/opt/tBotSheduler/.env

[Install]
WantedBy=multi-user.target
```

```bash
systemctl daemon-reload
systemctl enable tbotsheduler
systemctl start tbotsheduler
systemctl status tbotsheduler  # проверить
```

### Мониторинг
```bash
journalctl -u tbotsheduler -f  # логи в реальном времени
journalctl -u tbotsheduler --since "1 hour ago"  # логи за час
```

### Обновление
```bash
cd /opt/tBotSheduler
git pull
systemctl restart tbotsheduler
```

### Бэкап SQLite
```bash
# Ежедневный бэкап через cron
0 3 * * * cp /opt/tBotSheduler/bot.db /opt/tBotSheduler/backups/bot-$(date +\%Y\%m\%d).db
# Хранить 7 дней
0 3 * * * find /opt/tBotSheduler/backups/ -name "bot-*.db" -mtime +7 -delete
```

### Особенности Timeweb
- **IP адрес:** при пересоздании сервера IP меняется. Если используете webhook (не long polling), закрепите IP через дополнительный сервис или используйте домен
- **Снапшоты:** бесплатно — 1 снапшот на VPS. Делайте снапшот перед крупными обновлениями
- **Смена тарифа:** требует переустановки ОС. Данные на системном диске теряются — храните `bot.db` на отдельном томе, если планируете апгрейд
- **Мониторинг:** в панели Timeweb доступны графики CPU/RAM/disk. Настройте алерты при загрузке CPU > 80% и при заполнении диска > 85%
- **Поддержка:** тикеты через панель управления, время ответа — от 15 минут до 2 часов в рабочее время

FastAPI: http://localhost:8000
Бот запускается вместе с FastAPI (lifespan event)

## Тестирование

### Подход
**TDD (Test-Driven Development)** — тесты пишутся до реализации. Каждая фича начинается с падающего теста, который описывает ожидаемое поведение.

### Уровни тестов

```
┌─────────────────────────────────────┐
│           E2E (end-to-end)          │  ← бот + Web App + Telegram API (mock)
│   pytest + httpx + aioresponses     │
├─────────────────────────────────────┤
│       Интеграционные тесты          │  ← БД + JobQueue + хендлеры
│   pytest-asyncio + SQLite (:memory:) │
├─────────────────────────────────────┤
│          Юнит-тесты                 │  ← модели, валидация, утилиты
│   pytest (чистые функции)           │
└─────────────────────────────────────┘
```

### Стек тестирования

| Инструмент | Назначение |
|-----------|-----------|
| `pytest` | Основной раннер |
| `pytest-asyncio` | Асинхронные тесты (бот, FastAPI, БД) |
| `httpx.AsyncClient` | Тестирование FastAPI эндпоинтов |
| `aioresponses` / `respx` | Mock Telegram Bot API |
| `sqlite3 :memory:` | Интеграционные тесты БД (чистый старт каждый тест) |
| `pytest-cov` | Замер покрытия |
| `pytest-xdist` | Параллельный запуск тестов |

### Структура тестового проекта

```
tests/
├── conftest.py              # фикстуры: БД, бот, клиент, моки
├── unit/
│   ├── test_models.py       # тесты SQLAlchemy моделей (создание, связи, индексы)
│   ├── test_validators.py   # тесты валидации initData, anti-flood, прав
│   ├── test_scheduler.py    # тесты логики JobQueue + Heartbeat (чистая логика)
│   └── test_security.py     # тесты HMAC, проверки админа, уникальности индексов
├── integration/
│   ├── test_repository.py       # CRUD операции через репозиторий (с SQLite in-memory)
│   ├── test_handlers.py         # хендлеры бота с mock Telegram API
│   ├── test_api.py              # FastAPI эндпоинты (/health, /api/book, /api/cancel)
│   ├── test_notifications.py    # полный цикл: бронь → JobQueue → отправка → чек
│   ├── test_booking_flow.py     # сквозной сценарий: слот → бронь → отмена
│   ├── test_external_api.py     # REST /api/v1/* — auth, CRUD, permissions, rate limit (*)
│   └── test_webhook_delivery.py # вебхуки: dispatch, signing, retry, degradation (*)
├── e2e/
│   ├── test_full_scenario.py       # полный сценарий: админ → слот → юзер → бронь → уведомление → отмена
│   ├── test_race_conditions.py     # конкурентные бронирования, двойной клик
│   ├── test_error_scenarios.py     # сломанный initData, слот занят, бот упал и встал
│   └── test_external_e2e.py        # API-ключ → слот → бронь → вебхук → отмена (*)
│
└── (*) External API — реализуются после Milestone 5
```

### Фикстуры (conftest.py)

```python
@pytest.fixture
async def db():
    """SQLite in-memory БД для каждого теста — чистое состояние."""
    engine = create_async_engine("sqlite+aiosqlite://", echo=True)
    async with engine.begin() as conn:
        await conn.run_sync(Base.metadata.create_all)
    yield engine
    await engine.dispose()

@pytest.fixture
def mock_telegram_api():
    """Mock ответов Telegram Bot API (getChatAdministrators, sendMessage и т.д.)."""
    with aioresponses() as m:
        yield m

@pytest.fixture
async def app():
    """FastAPI app + бот (mock Telegram token)."""
    ...

@pytest.fixture
async def client(app):
    """Async HTTP клиент для тестов FastAPI."""
    async with AsyncClient(app=app, base_url="http://test") as ac:
        yield ac
```

### Критические сценарии для тестов

#### Юнит
- Создание всех моделей с валидными данными
- Уникальные индексы: дубликат брони, дубликат слота → ошибка
- Валидация initData: правильный hash → OK, неправильный → None
- Anti-flood: 1 запрос → OK, 2 запроса за 3 сек → блок

#### Интеграционные
- CRUD: создать слот → забронировать → прочитать бронь → отменить
- JobQueue: регистрация задачи → запуск в нужное время → отправка
- Heartbeat: запуск `check_pending()` с просроченными → отправляются
- FastAPI healthcheck: `/health` → 200 OK

#### E2E
- Полный сценарий: админ логинится → создаёт слоты → broadcast → юзер бронирует → уведомление → отмена
- Race condition: 5 одновременных броней на 1 слот → проходит только 1
- Восстановление после падения: бот упал → перезапустился → Heartbeat догнал уведомления

### Покрытие

**Цель:** минимум 80% покрытия кода (lines + branches) для production-модулей.

Измеряется `pytest-cov`, проверяется в CI:

```bash
pytest --cov=app --cov-report=term-missing --cov-fail-under=80
```

### TDD-цикл для каждой фичи

```
1. Написать тест (падает — RED)
2. Написать минимальную реализацию (тест проходит — GREEN)
3. Рефакторинг кода (тест всё ещё проходит — REFACTOR)
```

### Что не тестируется
- Статика Web App (HTML/CSS/JS) — визуальная верификация вручную
- Интеграция с реальным Telegram API (используем mock)
- Производительность под нагрузкой (отдельное нагрузочное тестирование)

