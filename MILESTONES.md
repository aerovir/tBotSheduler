...
- **TDD:** интеграционный тест healthcheck (200 OK)
...
**Цель**: Админ может создавать слоты и публиковать кнопку бронирования в канал.

**Задачи:**
- @check_admin — написать тест
- **TDD:** unit-тест проверки прав: админ (+кеш), не-админ, пользователь без прав
- Проверка прав админа через `getChatAdministrators` + кеш 5 мин
- **Безопасность:** декоратор `@check_admin()` для всех админ-команд
- **AuditLog:** логировать создание/удаление слотов
- Команда `/setup` — привязка к каналу
...
**Критерий приёмки**: Админ авторизован → создал слоты через Web App (запись в AuditLog) → в канале появилось сообщение с кнопкой. Не-админ получает отказ. `pytest --cov-fail-under=80` проходит.
