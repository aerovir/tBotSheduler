...
**Цель**: Админ может создавать слоты, публиковать кнопку бронирования в канал и делегировать права модераторам.

**Задачи:**
- @check_admin — написать тест
- **TDD:** unit-тест проверки прав: owner, moderator, не-админ, пользователь без прав
- **TDD:** unit-тест разграничения ролей: moderator не может вызвать /add_moderator
- **TDD:** интеграционный тест — owner добавляет moderator → moderator создаёт слот
- **TDD:** интеграционный тест — moderator вызывает /remove_moderator → отказ
- Проверка прав через таблицу `Admin` (owner/moderator) + кеш 5 мин
- **Безопасность:** декоратор `@check_admin()` для всех админ-команд
- **Безопасность:** декоратор `@check_role('owner')` для команд делегирования
- **AuditLog:** логировать создание/удаление слотов и добавление/удаление модераторов
...
**Критерий приёмки**: Owner создал слоты, добавил модератора, broadcast в канал. Модератор создал/удалил слот, но не может добавить другого модератора. Не-админ получает отказ. Healthcheck на `/health` показывает ok по всем узлам. `pytest --cov-fail-under=80` проходит.
