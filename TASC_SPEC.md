...
### #9 — change_booking() не атомарна (потеря брони)
...
| **Исправление** | Обернуть в единую транзакцию с savepoint или выполнять как atomic update. |
...
| **Исправление** | Проверка `if admin:` перед доступом к `.id`. |
...
### #16 — Единственная AsyncSession на все concurrent-запросы
...
### #17 — N+1 запросы в booking_service
...
### #18 — change_booking() теряет comment
...
### #19 — free_slot не уведомляет ожидающих
...
### #20 — join_waiting не проверяет занятость слота
...
### #21 — Очистка списка ожидания при ошибках отправки
...
### #22 — Мёртвый код в forgotten_service.py
...
### #23 — export_service берёт произвольную бронь
...
### #24 — Доступ к внутреннему API APScheduler
...
### #25 — Наивные datetime без timezone
...
| **P2** | #9, #12 | Потеря брони, AttributeError | 🟠 Остались 2 |
| **P3** | #16–#25 | Утечки, N+1, deprecated API | 🟡 |
