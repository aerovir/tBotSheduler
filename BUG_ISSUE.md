...
### #BUG-04 — Нет проверки `auth_date` в `validate_init_data`

| Аспект | Детали |
|--------|--------|
| **Где** | `core/security.py:27-49` |
| **Суть** | Функция не проверяет `auth_date`. Telegram рекомендует отклонять initData старше 24 часов |
| **Влияние** | Replay-атака: перехваченный initData можно воспроизводить бесконечно. Все 4 эндпоинта Web App уязвимы |
| **Приоритет** | P0 (из TASC #3) |
...
| Существующие P0 (не исправлены) | 3 | 🔴 #BUG-04, #BUG-05, #BUG-07 |
| Косметика | 1 | 🔵 #BUG-08 |

**Главный вывод:** #BUG-01 (shared-сессия) исправлен через Option E. #BUG-06 (None checks) исправлен. Оставшиеся P0: auth_date (#BUG-04), race condition (#BUG-05), heartbeat (#BUG-07).
